...
export default async function render(props, { image }) {
  ...
  const stat = (name, value) => box({ width: 170, height: 30, margin: 1 }, text(name, { width: 100, padding: '5px 8px', color: '#fff', backgroundColor: 'rgba(0,0,0,.8)' }), text(value, { width: 70, padding: '5px 8px', color: '#111', backgroundColor: 'rgba(239,238,239,.8)' }));
  const data = props.op ?? {};
  const panel = (title, children, style = {}) => box({ width: 600, marginBottom: 8, flexDirection: 'column', backgroundColor: 'rgba(63,63,62,.93)', color: '#fff', ...style }, box({ height: 28, padding: '5px 10px', alignItems: 'center', backgroundColor: 'rgba(0,0,0,.8)' }, text(title, { fontWeight: 600 })), ...children);
  return box({ width: 1200, height: 800, position: 'relative', overflow: 'hidden', backgroundImage: `url(${background})`, backgroundSize: 'cover', fontFamily: 'NotoSansHans' },
    ...
    box({ position: 'absolute', left: 10, bottom: 20, width: 330, flexDirection: 'column', color: '#111', backgroundColor: 'rgba(239,238,239,.55)' }, text(data.profession, { fontSize: 18, padding: 8 }), text(data.tags, { padding: 8 }), text(props.professionBranch?.name, { padding: 8 }), text(props.professionBranch?.desc, { padding: 8 }), text(data.name, { fontSize: 30, fontWeight: 600, padding: 8 }), text(data.nameEn, { padding: '0 8px 8px' })),
    box({ position: 'absolute', top: 20, right: 10, width: 600, flexDirection: 'column' },
      panel('天赋', (props.talents ?? []).map((item) => box({ padding: 6, backgroundColor: 'rgba(176,177,177,.7)' }, text(item.evolve, { width: 100 }), text(item.name, { width: 160 }), text(item.desc, { flex: 1 })))),
      panel('基建技能', (props.buildingSkills ?? []).map((item, index) => box({ padding: 6, alignItems: 'center', backgroundColor: 'rgba(176,177,177,.7)' }, index === 0 ? h('img', { src: building, width: 36, height: 36 }) : null, text(item.name, { marginLeft: 8, width: 130 }), text(item.desc, { flex: 1 })))),
